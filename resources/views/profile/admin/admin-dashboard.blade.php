    ...
    <body class="font-sans text-gray-900 antialiased bg-body-blue flex justify-center items-center min-h-screen">
        ...
        <div class="flex flex-col md:flex-row md: md:mt-[4rem] p-4">
            ...
            <div class="w-full md:w-[32rem] p-7 bg-custom-blue rounded-xl shadow-xl mb-4 md:mb-0">
                <div class="mt-1 p-6 border-4 border-white rounded-2xl min-h-full">
                    ...
                    <script>
                        document.addEventListener('DOMContentLoaded', function() {
                            ...
                            function capturePhoto() {
                                ...
                                // Set canvas size to match camera container size
                                photoCanvas.width = cameraContainer.clientWidth;
                                photoCanvas.height = cameraContainer.clientHeight;

                                // Calculate scaling factors to fit the video inside the container
                                ...
                                camera.classList.add('hidden'); // Hide the camera after capturing
                            }
                            ...
                            takePhotoBtn.addEventListener('click', (event) => {
                                ...
                                if (!isTakingPhoto) {
                                    // Show the camera and start streaming
                                    cameraContainer.classList.remove('hidden');
                                    photoCanvas.classList.add('hidden'); // Hide the canvas initially
                            ...
                            retryBtn.addEventListener('click', (event) => {
                                event.preventDefault(); // Prevent form submission or page reload
                                photoCanvas.classList.add('hidden'); // Hide the captured photo
                                camera.classList.remove('hidden'); // Show the camera again
                                cameraContainer.classList.remove('hidden');
                        ...
                        <div class="flex flex-col gap-4 mt-10">
                            ...
                            <!-- <form action="{{ route('attendance.saveRemarks') }}" method="POST">
                                ...
                                <div class="flex flex-col mb-4">
                                    <label for="remarks" class="text-white mb-2">Remarks</label>
                                    <input id="remarks" name="remarks" type="text" placeholder="Enter your remarks here" class="w-full px-4 py-2 rounded-lg text-gray-800">
                                </div>
                            </form> -->

                            <!-- Time Out Form -->
                            <form action="{{ route('attendance.admintimeIn') }}" method="POST">
                                @csrf
                            ...
                            <form action="{{ route('attendance.admintimeOut') }}" method="POST">
                                @csrf
